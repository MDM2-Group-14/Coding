...
def runsimulation(pitchOvershoot, yawOvershoot, dt):
    ...
    thrust1 = np.array([0.05, 0.2, -0.05])
    thrust2 = np.array([0.15, -0.3, 0.5])
    thrust3 = np.array([-0.125, 0.05, -0.125])
    thrust4 = np.array([-0.05, -0.075, -0.075])

    thrustDelayCounter = 10
    ...
    for t in np.linspace(dt, hours, steps):
        ...
        if thrustDelayCounter < 10:
            # thrust = ((0.5 - ((thrustDelayCounter)**-1))*(thrust - lastThrust))+lastThrust
            thrust = (0.05 * thrustDelayCounter) * (thrust - lastThrust) + lastThrust
            thrustDelayCounter += 1
